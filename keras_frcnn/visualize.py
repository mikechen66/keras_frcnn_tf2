...
the file supports both cv2 or PIL library, with separately methods
"""
import cv2
import numpy as np
import colorsys
...
def draw_boxes_and_label_on_image_cv2(img, class_label_map, class_boxes_map):
    """
    The method using cv2 to show boxes on image with various class labels
    :param img:
    ...
    for c, boxes in class_boxes_map.items():
        for box in boxes:
            ...
            # Check box order is bb_left, bb_top, bb_width, bb_height and make sure all box should be int for OpenCV
            bb_left = int(box[0])
            bb_top = int(box[1])
            bb_width = int(box[2])
            bb_height = int(box[3])

            # Please notice that prob will round 2 digits
            prob = round(box[4], 2)
            ...
            # The rectangle for fill text rect
            cv2.rectangle(img, (text_org[0] - 5, text_org[1] + base_line - 5),
